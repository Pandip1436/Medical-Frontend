...
// IndiaMART Push API card. Flow:
//   1. Admin clicks "Generate webhook URL" → backend mints a random token,
//      stores it, returns the full URL.
//   2. Admin copies the URL into seller.indiamart.com → Lead Manager →
//      Push API and confirms via OTP.
//   3. From then on, every new lead lands in /crm/leads automatically.
...
export function IndiamartCard() {
  ...
  const handleTestPush = async () => {
    try {
      ...
      toast.success(
        r.unique_query_id
          ? `Test lead created (UNIQUE_QUERY_ID ${r.unique_query_id}). Check /crm/leads.`
  ...
  return (
    <Card>
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">
              Receives leads from your IndiaMART seller account in real-time.
              Every new inquiry (Direct, Buy Lead, PNS, Catalog, WhatsApp) lands
              in /crm/leads within seconds. Dedup is automatic.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-5">
        ...
        {connected && (
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
            {status?.lastReceivedAt ? (
              ...
              <span>Awaiting first push from IndiaMART…</span>
            )}
            {status?.createdAt && (
              <span>
                Connected{' '}
          ...
          <div className="flex items-start gap-2 rounded-lg border border-amber-300/60 bg-amber-500/[0.07] px-3 py-2.5 text-xs text-amber-800 dark:border-amber-700/40 dark:text-amber-300">
            ...
            <div>
              ...
              <p className="mt-0.5 text-amber-700/80 dark:text-amber-400/80">
                IndiaMART may not have completed activation. Double-check the
                URL is pasted correctly and the OTP was confirmed.
              </p>
            </div>
          </div>
        )}

        {/* Not-connected: big "Generate" CTA */}
        {!connected && (
          <div className="rounded-xl border border-dashed border-border bg-muted/20 px-4 py-6 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              ...
              into IndiaMART's seller panel and confirm via OTP.
            </p>
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
              ...
              <Button
                ...
                disabled={testing}
                className="gap-1.5"
                title="Mints a URL (if missing) then runs a sample lead through the receiver — no IndiaMART account needed."
              >
                {testing ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                ) : (
        ...
        {/* Connected: URL display + paste instructions */}
        {connected && url && (
          <div className="space-y-3">
            ...
            <details className="rounded-lg border border-border/60 bg-muted/20 p-3 text-xs">
              <summary className="cursor-pointer font-semibold text-foreground">
                How to activate it on IndiaMART (one-time)
              </summary>
              <ol className="mt-2 space-y-1.5 pl-4 list-decimal text-muted-foreground">
                <li>
                  Open{' '}
                  <a
                    href="https://seller.indiamart.com/leadmanager/crmpushapi"
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-0.5 text-primary hover:underline"
                  >
                    seller.indiamart.com → Lead Manager → Push API
                    <ExternalLink className="h-3 w-3" />
                  </a>
                </li>
                <li>
                  Choose <strong>Other</strong> as your CRM platform (or pick
                  one from the dropdown — doesn't change behavior).
                </li>
                <li>Paste the URL above into the "Webhook Listener URL" field.</li>
                <li>Confirm the OTP IndiaMART sends to your registered mobile.</li>
                <li>
                  Activation is instant. Past leads from before activation are
                  not sent — only leads that arrive after this moment.
                </li>
              </ol>
            </details>
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <Button
                ...
                disabled={testing}
                className="gap-1.5"
                title="Pushes a sample lead through the receiver — verifies the integration without needing IndiaMART."
              >
                {testing ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                ) : (
...
function StatusPill({ connected, stale }: { connected: boolean; stale: boolean }) {
  ...
  return (
    <span className={cn(
      'inline-flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-2 py-1 text-[11px] font-semibold text-emerald-700 dark:text-emerald-400'
    )}>
