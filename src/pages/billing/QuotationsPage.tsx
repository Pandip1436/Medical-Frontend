...
export default function QuotationsPage() {
  ...
  const periodQuotations = useMemo(() => {
    ...
    const now = new Date()
    // Compare on the LOCAL calendar day, not the UTC day. qt.date is a UTC ISO
    ...
    switch (period) {
      case 'today':
        result = result.filter((qt) => localDay(qt.date) === todayStr)
        break
      case 'week': {
        const weekStr = weekStartISO(now)
        result = result.filter((qt) => localDay(qt.date) >= weekStr)
        break
      }
      case 'month': {
        const monthStart = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-01`
        result = result.filter((qt) => localDay(qt.date) >= monthStart)
        break
      }
      case 'custom':
        if (dateFrom) result = result.filter((qt) => localDay(qt.date) >= dateFrom)
        if (dateTo) result = result.filter((qt) => localDay(qt.date) <= dateTo)
        break
    }
    return result
  ...
  return (
    ...
    >
      ...
      >
        {/* All filters on one flex-wrap row so Status/Customer + the custom
        ...
        <div className="col-span-full flex flex-wrap items-end gap-4">
          ...
          <div className="flex shrink-0 flex-col justify-end gap-1.5">
            ...
            <ColumnsToggle columns={QUOTATION_COLUMNS} visible={cols.visible} onToggle={cols.toggle} onReset={cols.reset} />
          </div>
        </div>
      </DataTableFilterBar>
