...
// Small delta indicator for the compact KPI strip — same up/down/flat mapping
// as the dashboard's own KpiTile, just without pulling in that component's
// taller card chrome.
...
interface LedgerDetailContentProps {
  partyId: string
  partyType: 'customer' | 'supplier'
}

export function LedgerDetailContent({ partyId, partyType }: LedgerDetailContentProps) {
  ...
  const [partyDetail, setPartyDetail] = useState<Customer | Supplier | null>(null)

  // Force both fetch effects to re-run on branch change so the page reflects
  // the new branch's data without the user having to re-pick the party.
  useBranchRefresh(() => setRefreshKey((k) => k + 1))

  // Party detail — fetched fresh from partyId/partyType/refreshKey alone, no
  // cross-render caching needed (the parent split view already only mounts
  // this component while a party is selected, and remounts it — new state —
  // whenever partyId changes, since it's keyed on that id there).
  useEffect(() => {
    let cancelled = false
    api.get(partyType === 'customer' ? `/customers/${partyId}` : `/suppliers/${partyId}`)
      .then((res) => { if (!cancelled) setPartyDetail(res.data) })
      .catch(() => { /* leave detail null; ledger still loads by id */ })
    return () => { cancelled = true }
  }, [partyId, partyType, refreshKey])

  useEffect(() => {
    const endpoint = partyType === 'customer'
      ? `/reports/financial/ledger/${partyId}`
      : `/reports/financial/supplier-ledger/${partyId}`
  ...
  const selectedPartyName = partyDetail?.name ?? ''
  ...
  }, [ledgerWithBalance.length, partyId, activeTab])
  ...
  const periodSwitcher = (
    <div className="flex flex-nowrap items-center gap-2 overflow-x-auto pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      ...
      {/* Month navigator — always rendered (reserving its width) so toggling
          Year/Month doesn't shrink the switcher and shift everything sharing
          this row. Outside Month it stays visible but goes inert/blank: the
          month name clears and prev/next disable, rather than disappearing. */}
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        <Popover open={monthPopoverOpen} onOpenChange={setMonthPopoverOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              size="sm"
              className="h-9 shrink-0 px-2.5 font-medium min-w-14 sm:px-3 sm:min-w-20"
              disabled={selectedPeriod !== 'month'}
            >
              {selectedPeriod === 'month' ? MONTH_NAMES[selectedMonthIdx] : '—'}
            </Button>
          </PopoverTrigger>
        ...
        <Button
          ...
          className="h-9 w-9"
          onClick={goNextMonth}
          disabled={selectedPeriod !== 'month' || cannotGoNextMonth}
  ...
  const partyTypeLabel = partyType === 'customer' ? 'Customer' : 'Supplier'
  const outstanding = (partyDetail as any)?.currentOutstanding ?? 0
  const creditLimit =
    partyType === 'customer'
      ? ((partyDetail as Customer | null)?.creditLimit ?? 0)
      : 0
  const creditUsagePct =
    creditLimit > 0 ? Math.min(100, Math.max(0, (outstanding / creditLimit) * 100)) : 0
  const phone =
    (partyDetail as any)?.phone ?? ''
  const subMeta =
    partyType === 'customer'
      ? (partyDetail as Customer | null)?.gstin
      : (partyDetail as Supplier | null)?.contactPerson

  return (
    // The shell's right panel has no scroll container of its own (each
    // detailContent owns its own, matching CustomerDetailContent/
    // SupplierDetailContent's convention) — without this, content taller
    // than the panel just clips instead of scrolling.
    ...
    >
      ...
      <Card
        className={cn(
          ...
          partyType === 'customer' ? 'border-l-sky-500' : 'border-l-purple-500',
        )}
      >
        <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge
                  variant={partyType === 'customer' ? 'info' : 'purple'}
                  size="sm"
                >
                  {partyTypeLabel}
                </Badge>
          ...
          <div className="shrink-0 sm:text-right sm:ml-auto">
            ...
            {partyType === 'customer' && creditLimit > 0 && (
              <div className="mt-1.5 w-32 sm:ml-auto">
      ...
      {/* ── KPI Strip — compact single-row cards (icon + label/value inline,
          delta+subtitle below) instead of dashboard's taller KpiTile, which
          was eating a disproportionate amount of the panel's height here ── */}
      <div className="grid grid-cols-2 gap-2 sm:gap-2.5 lg:grid-cols-4">
        {isLoading
          ? Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="rounded-lg border border-border/80 bg-background p-2.5 shadow-sm shadow-black/[0.02]">
          ...
          : kpis.map((kpi) => {
            ...
            return (
              <div key={kpi.key} className="rounded-lg border border-border/80 bg-background p-2.5 shadow-sm shadow-black/[0.02]">
                <div className="flex items-center gap-2">
                  <div className={cn('flex h-6 w-6 shrink-0 items-center justify-center rounded-md', kpi.iconBg)}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate font-mono text-sm font-bold tabular-nums">
                      {isCurrency ? formatCurrency(kpi.value) : kpi.value}
                    </p>
                  </div>
                </div>
                {(kpi.delta || kpi.subtitle) && (
                  <div className="mt-1 flex items-center gap-1 text-[10px] text-muted-foreground">
                    {kpi.delta && <KpiDeltaChip delta={kpi.delta} />}
      ...
      {/* ── Tabs: Ledger | Trend — period switcher shares this row (pushed to
          the far right on desktop via ml-auto; wraps below on mobile where
          the tabs' own grid already claims the full row width) ── */}
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'ledger' | 'trend')}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="sm:ml-auto">{periodSwitcher}</div>
        </div>
