...
import { Phone, MessageCircle, Mail, StickyNote, Clock } from 'lucide-react'
...
export type SupplierActivityType = 'CALL' | 'WHATSAPP' | 'EMAIL' | 'NOTE' | 'REMINDER'
export type SupplierActivityStatus = 'PENDING' | 'DONE' | 'CANCELLED'
...
export interface SupplierActivity {
  ...
  dueAt?: string | null
  status?: SupplierActivityStatus | null
...
// Build a single schema that conditionally requires fields based on `type`.
// Using `superRefine` lets us keep one form shape across all 5 sub-types.
const formSchema = z
  .object({
...
export type SupplierActivityFormValues = z.input<typeof formSchema>
...
export interface SupplierActivityPayload {
  type: SupplierActivityType
  notes?: string
  title?: string
  contactName?: string
...
interface Props {
  ...
  type: SupplierActivityType
  editing: SupplierActivity | null
  onSubmit: (payload: SupplierActivityPayload) => Promise<void> | void
}

export function SupplierActivityDialog({
  open,
  onOpenChange,
  type,
  editing,
  ...
  const defaults: SupplierActivityFormValues = useMemo(
    () => ({
      type,
      notes: '',
      title: '',
      contactName: '',
      subject: '',
      occurredAt: type === 'REMINDER' ? '' : toDatetimeLocal(new Date().toISOString()),
      dueAt: '',
      status: 'PENDING',
    }),
    [type],
  ...
  } = useForm<SupplierActivityFormValues>({
  ...
  async function handleFormSubmit(data: SupplierActivityFormValues) {
    try {
      const payload: SupplierActivityPayload = {
        type: data.type,
      }
      if (data.type === 'REMINDER') {
        ...
        if (data.type === 'EMAIL' && data.subject?.trim()) payload.subject = data.subject.trim()
      }
      ...
      <DialogContent className="max-w-lg rounded-2xl">
        <DialogHeader>
          <div className="flex items-center gap-2">
            <span className={`inline-flex h-7 w-7 items-center justify-center rounded-md border ${meta.chip}`}>
          ...
          <DialogDescription>
            {type === 'REMINDER'
              ? 'Schedule a future follow-up — it will show in the timeline as Pending until marked Done.'
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          {type === 'REMINDER' ? (
            <>
              <div className="space-y-2">
                ...
                <Input placeholder="e.g. Follow up on June invoice" {...register('title')} />
                {errors.title && (
              ...
              <div className="space-y-2">
                ...
                <Textarea placeholder="Extra context for this reminder" rows={3} {...register('notes')} />
              </div>
            </>
          ) : (
            <>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  placeholder={
                    type === 'NOTE'
                      ? 'Free-text note about this supplier'
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'Saving…' : editing ? `Update ${meta.label}` : `Log ${meta.label}`}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
